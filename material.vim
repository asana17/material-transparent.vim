...
function! s:SetHighlightsp(group, fg, bg, attr, sp)
  let l:attr = a:attr
...
let s:skyblue= { 'gui': '#426abf', 'cterm': 111 }
...
if g:material_theme_style == 'palenight' || g:material_theme_style == 'palenight-community'
  ...
  let s:guides = { 'gui': '#5d5f73', 'cterm': 17 }
  "let s:line_numbers = { 'gui': '#3a3f58', 'cterm': 60 }
  let s:line_numbers = { 'gui': '#5d5f73', 'cterm': 60 }
...
call s:SetHighlight('CursorColumn', s:white, s:bg, '')
"call s:SetHighlight('CursorLine', '', s:line_highlight, '')
call s:SetHighlight('CursorLine', s:white, s:bg, '')
...
call s:SetHighlight('ErrorMsg', s:white, s:red, 'bold')
...
call s:SetHighlight('MatchParen', s:cyan, s:bg, 'bold')
...
call s:SetHighlightsp('SpellBad', s:red, '', 'undercurl', s:cyan)
...
call s:SetHighlight('TabLineSel', s:white, s:cyan, '')
...
"call s:SetHighlight('WarningMsg', s:cyan, '', '')
"call s:SetHighlight('WildMenu', s:bg, s:cyan, '')
call s:SetHighlight('WildMenu', s:white, s:cyan, '')
"call s:SetHighlight('NormalNC', s:cyan, s:bg, '')
...
call s:SetHighlight('Error', s:white, s:purple, '')
...
" TODO! LSP color setting
call s:SetHighlight('LspHintText', s:cyan, '', '')
call s:SetHighlight('LspErrorHighlight', s:skyblue, s:purple, '')
